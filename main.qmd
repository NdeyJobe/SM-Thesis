...
# Example: Assuming 'person' is your dataset and 'index_date' is the column of interest
# Ensure the index_date column is in Date format
person <- person %>%
...
# Data Input for 2-Year Time Horizon
data_2year <- data.frame(
  # Risk_Group: Each group includes Diabetic, Hypertensive, CKD Medium Risk, CKD High Risk, CKD Very High Risk
  Risk_Group = rep(c("Diabetic", "Hypertensive", "CKD Medium Risk", "CKD High Risk", "CKD Very High Risk"), each = 8),
  
  # Population groups: All Populations, Asian Population, Black Population, White Population
  Population = rep(c("All Populations", "Asian Population", "Black Population", "White Population"), times = 10),
  
  # Model_Comparison: Alternates between CKD-EPI 2021 vs CKD-EPI 2009 and CKD-EPI 2021 vs MDRD
  Model_Comparison = rep(c("CKD-EPI 2021 vs CKD-EPI 2009", "CKD-EPI 2021 vs MDRD"), each = 4, times = 5),
  
  # Difference in Harrell's C-statistic
  # For each group, the first 4 rows = "2021 vs 2009", and the next 4 rows = "2021 vs MDRD"
  Difference = c(
    # Diabetic: CKD-EPI 2021 vs CKD-EPI 2009
    0.02, 0.03, 0.04, -0.01, 
    # Diabetic: CKD-EPI 2021 vs MDRD
    -0.10, -0.05, 0.00, 0.10, 

    # Hypertensive: CKD-EPI 2021 vs CKD-EPI 2009
    -0.01, -0.02, 0.00, 0.02, 
    # Hypertensive: CKD-EPI 2021 vs MDRD
    -0.20, -0.15, -0.10, -0.05,

    # CKD Medium Risk: CKD-EPI 2021 vs CKD-EPI 2009
    0.05, 0.04, 0.03, 0.02, 
    # CKD Medium Risk: CKD-EPI 2021 vs MDRD
    0.00, 0.01, -0.01, -0.02,

    # CKD High Risk: CKD-EPI 2021 vs CKD-EPI 2009
    0.06, 0.07, 0.08, 0.05, 
    # CKD High Risk: CKD-EPI 2021 vs MDRD
    0.02, 0.04, 0.03, 0.01,

    # CKD Very High Risk: CKD-EPI 2021 vs CKD-EPI 2009
    -0.017, 0.056, 0.044, 0.084, 
    # CKD Very High Risk: CKD-EPI 2021 vs MDRD
    -0.030, 0.050, 0.070, 0.080
  ),
  
  # Lower Confidence Interval for each Difference
  CI_Lower = c(
    # Diabetic: 2021 vs 2009
    0.00, 0.01, 0.02, -0.03, 
    # Diabetic: 2021 vs MDRD
    -0.15, -0.10, -0.05, 0.05, 

    # Hypertensive: 2021 vs 2009
    -0.03, -0.04, -0.02, 0.00, 
    # Hypertensive: 2021 vs MDRD
    -0.25, -0.20, -0.15, -0.10,

    # CKD Medium Risk: 2021 vs 2009
    0.03, 0.02, 0.01, 0.00, 
    # CKD Medium Risk: 2021 vs MDRD
    -0.02, -0.01, -0.03, -0.05,

    # CKD High Risk: 2021 vs 2009
    0.04, 0.05, 0.06, 0.03, 
    # CKD High Risk: 2021 vs MDRD
    0.00, 0.02, 0.01, -0.01,

    # CKD Very High Risk: 2021 vs 2009
    -0.03, 0.03, 0.02, 0.05, 
    # CKD Very High Risk: 2021 vs MDRD
    -0.04, 0.02, 0.04, 0.06
  ),
  
  # Upper Confidence Interval for each Difference
  CI_Upper = c(
    # Diabetic: 2021 vs 2009
    0.04, 0.05, 0.06, 0.01, 
    # Diabetic: 2021 vs MDRD
...
# Function to generate plots for a given dataset and time horizon
plot_superiority <- function(data, time_horizon) {
  ggplot(data, aes(x = Difference, y = Risk_Group, color = Model_Comparison)) +
    ...
    labs(
      title = paste("Superiority Test Across Populations For", time_horizon, "Prediction"),
      x = "Difference in C-Statistic",
      y = "Risk Group",
...
# Generate the 2-Year
plot_2year <- plot_superiority(data_2year, "2-Year")
plot_2year
```
...
cat_vars <- c("Disease_Status", "sex_at_birth", "Health_Insurance",
...
# Create Table 1 as before using the `CreateTableOne` function
table1 <- CreateTableOne(vars = c(cat_vars, cont_vars), strata = "race", data = table1_data, factorVars = cat_vars)

# Convert Table 1 to a data frame for customization
...
# Assuming `table1` is your CreateTableOne object
table1_df <- print(table1, showAllLevels = TRUE, printToggle = FALSE)

# Save the table data as an R object
save(table1_df, file = "table1_data.RData")

# Optionally, save it as a CSV for easy review
write.csv(table1_df, "table1_data.csv", row.names = FALSE)

# Load the saved R object (if needed)
# load("table1_data.RData")
...
# Format the table with extra spacing
formatted_table <- kable(
  table1_df, 
  format = "html", 
  escape = FALSE, 
  caption = "Table 1: Baseline Characteristics Stratified by Race"
) %>%
  kable_styling(
    full_width = FALSE, 
    position = "center",
  ...
  row_spec(1:nrow(table1_df), extra_css = "padding-top: 12px; padding-bottom: 12px;") %>% 
  column_spec(1, bold = TRUE)  # Bold first column for emphasis

# Save the table as an HTML file temporarily
temp_html <- "table1_temp.html"
save_kable(formatted_table, file = temp_html)
...
# Step 4: Identify predictor variables
predictor_vars <- setdiff(names(train_data), c("time_to_event", "disease_status", "person_id", "race"))

# Step 5: Transform numeric variables and convert binary variables to factors
train_data <- train_data %>%
