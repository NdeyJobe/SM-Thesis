...
# This query represents dataset "cohort data" for domain "person" and was generated for All of Us Controlled Tier Dataset v7
dataset_68236488_person_sql <- paste("
    SELECT
        person.person_id,
...
person_68236488_path <- file.path(
  Sys.getenv("WORKSPACE_BUCKET"),
  "bq_exports",
  Sys.getenv("OWNER_EMAIL"),
  strftime(lubridate::now(), "%Y%m%d"),  # Comment out this line if you want the export to always overwrite.
  "person_68236488",
  "person_68236488_*.csv")
message(str_glue('The data will be written to {person_68236488_path}. Use this path when reading ',
...
bq_table_save(
  bq_dataset_query(Sys.getenv("WORKSPACE_CDR"), dataset_68236488_person_sql, billing = Sys.getenv("GOOGLE_PROJECT")),
  person_68236488_path,
  destination_format = "CSV")


# Read the data directly from Cloud Storage into memory.
# NOTE: Alternatively you can `gsutil -m cp {person_68236488_path}` to copy these files
#       to the Jupyter disk.
...
dataset_68236488_person_df <- read_bq_export_from_workspace_bucket(person_68236488_path)

dim(dataset_68236488_person_df)

head(dataset_68236488_person_df, 5)
library(tidyverse)
library(bigrquery)

# This query represents dataset "cohort data" for domain "observation" and was generated for All of Us Controlled Tier Dataset v7
dataset_68236488_observation_sql <- paste("
    SELECT
...
#       But data exported on a different days will write to a new location so that historical
#       copies can be kept as the dataset definition is changed.
observation_68236488_path <- file.path(
  Sys.getenv("WORKSPACE_BUCKET"),
  "bq_exports",
  Sys.getenv("OWNER_EMAIL"),
  strftime(lubridate::now(), "%Y%m%d"),  # Comment out this line if you want the export to always overwrite.
  "observation_68236488",
  "observation_68236488_*.csv")
message(str_glue('The data will be written to {observation_68236488_path}. Use this path when reading ',
                 'the data into your notebooks in the future.'))
...
#       just read data from the CSVs in Cloud Storage.
bq_table_save(
  bq_dataset_query(Sys.getenv("WORKSPACE_CDR"), dataset_68236488_observation_sql, billing = Sys.getenv("GOOGLE_PROJECT")),
  observation_68236488_path,
  destination_format = "CSV")


# Read the data directly from Cloud Storage into memory.
# NOTE: Alternatively you can `gsutil -m cp {observation_68236488_path}` to copy these files
#       to the Jupyter disk.
read_bq_export_from_workspace_bucket <- function(export_path) {
...
dataset_68236488_observation_df <- read_bq_export_from_workspace_bucket(observation_68236488_path)

dim(dataset_68236488_observation_df)

head(dataset_68236488_observation_df, 5)
library(tidyverse)
library(bigrquery)

# This query represents dataset "cohort data" for domain "procedure" and was generated for All of Us Controlled Tier Dataset v7
dataset_68236488_procedure_sql <- paste("
    SELECT
        procedure.person_id,
...
#       But data exported on a different days will write to a new location so that historical
#       copies can be kept as the dataset definition is changed.
procedure_68236488_path <- file.path(
  Sys.getenv("WORKSPACE_BUCKET"),
  "bq_exports",
  Sys.getenv("OWNER_EMAIL"),
  strftime(lubridate::now(), "%Y%m%d"),  # Comment out this line if you want the export to always overwrite.
  "procedure_68236488",
  "procedure_68236488_*.csv")
message(str_glue('The data will be written to {procedure_68236488_path}. Use this path when reading ',
                 'the data into your notebooks in the future.'))
...
#       just read data from the CSVs in Cloud Storage.
bq_table_save(
  bq_dataset_query(Sys.getenv("WORKSPACE_CDR"), dataset_68236488_procedure_sql, billing = Sys.getenv("GOOGLE_PROJECT")),
  procedure_68236488_path,
  destination_format = "CSV")


# Read the data directly from Cloud Storage into memory.
# NOTE: Alternatively you can `gsutil -m cp {procedure_68236488_path}` to copy these files
#       to the Jupyter disk.
read_bq_export_from_workspace_bucket <- function(export_path) {
...
dataset_68236488_procedure_df <- read_bq_export_from_workspace_bucket(procedure_68236488_path)

dim(dataset_68236488_procedure_df)

head(dataset_68236488_procedure_df, 5)
library(tidyverse)
library(bigrquery)

# This query represents dataset "cohort data" for domain "condition" and was generated for All of Us Controlled Tier Dataset v7
dataset_68236488_condition_sql <- paste("
    SELECT
...
#       But data exported on a different days will write to a new location so that historical
#       copies can be kept as the dataset definition is changed.
condition_68236488_path <- file.path(
  Sys.getenv("WORKSPACE_BUCKET"),
  "bq_exports",
  Sys.getenv("OWNER_EMAIL"),
  strftime(lubridate::now(), "%Y%m%d"),  # Comment out this line if you want the export to always overwrite.
  "condition_68236488",
  "condition_68236488_*.csv")
message(str_glue('The data will be written to {condition_68236488_path}. Use this path when reading ',
                 'the data into your notebooks in the future.'))
...
#       just read data from the CSVs in Cloud Storage.
bq_table_save(
  bq_dataset_query(Sys.getenv("WORKSPACE_CDR"), dataset_68236488_condition_sql, billing = Sys.getenv("GOOGLE_PROJECT")),
  condition_68236488_path,
  destination_format = "CSV")


# Read the data directly from Cloud Storage into memory.
# NOTE: Alternatively you can `gsutil -m cp {condition_68236488_path}` to copy these files
#       to the Jupyter disk.
read_bq_export_from_workspace_bucket <- function(export_path) {
...
dataset_68236488_condition_df <- read_bq_export_from_workspace_bucket(condition_68236488_path)

dim(dataset_68236488_condition_df)

head(dataset_68236488_condition_df, 5)
library(tidyverse)
library(bigrquery)

# This query represents dataset "cohort data" for domain "measurement" and was generated for All of Us Controlled Tier Dataset v7
dataset_68236488_measurement_sql <- paste("
    SELECT
            ...
            (
                ...
                JOIN
                    ...
                    FROM
                        `cb_criteria` cr       
                    WHERE
                        concept_id IN (3000067, 3000185, 3000819, 3001122, 3001802, 3002302, 3002400, 3002812, 3004249, 3004410, 3004789, 3005715, 3006032, 3006453, 3007359, 3013801, 3016662, 3016723, 3017797, 3019510, 3019574, 3019839, 3019897, 3021044, 3021347, 3022192, 3022844, 3023421, 3023520, 3027945, 3034485, 3037121, 3037556, 3042733, 37022114, 37040063, 4235028, 46236952)       
                        AND full_text LIKE '%_rank1]%'      ) a 
                        ON (c.path LIKE CONCAT('%.', a.id, '.%') 
                        OR c.path LIKE CONCAT('%.', a.id) 
                        OR c.path LIKE CONCAT(a.id, '.%') 
...
#       But data exported on a different days will write to a new location so that historical
#       copies can be kept as the dataset definition is changed.
measurement_68236488_path <- file.path(
  Sys.getenv("WORKSPACE_BUCKET"),
  "bq_exports",
  Sys.getenv("OWNER_EMAIL"),
  strftime(lubridate::now(), "%Y%m%d"),  # Comment out this line if you want the export to always overwrite.
  "measurement_68236488",
  "measurement_68236488_*.csv")
message(str_glue('The data will be written to {measurement_68236488_path}. Use this path when reading ',
                 'the data into your notebooks in the future.'))
...
#       just read data from the CSVs in Cloud Storage.
bq_table_save(
  bq_dataset_query(Sys.getenv("WORKSPACE_CDR"), dataset_68236488_measurement_sql, billing = Sys.getenv("GOOGLE_PROJECT")),
  measurement_68236488_path,
  destination_format = "CSV")


# Read the data directly from Cloud Storage into memory.
# NOTE: Alternatively you can `gsutil -m cp {measurement_68236488_path}` to copy these files
#       to the Jupyter disk.
read_bq_export_from_workspace_bucket <- function(export_path) {
...
dataset_68236488_measurement_df <- read_bq_export_from_workspace_bucket(measurement_68236488_path)

dim(dataset_68236488_measurement_df)

head(dataset_68236488_measurement_df, 5)
library(tidyverse)
library(bigrquery)

# This query represents dataset "cohort data" for domain "device" and was generated for All of Us Controlled Tier Dataset v7
dataset_68236488_device_sql <- paste("
    SELECT
        device.person_id,
...
#       But data exported on a different days will write to a new location so that historical
#       copies can be kept as the dataset definition is changed.
device_68236488_path <- file.path(
  Sys.getenv("WORKSPACE_BUCKET"),
  "bq_exports",
  Sys.getenv("OWNER_EMAIL"),
  strftime(lubridate::now(), "%Y%m%d"),  # Comment out this line if you want the export to always overwrite.
  "device_68236488",
  "device_68236488_*.csv")
message(str_glue('The data will be written to {device_68236488_path}. Use this path when reading ',
                 'the data into your notebooks in the future.'))
...
#       just read data from the CSVs in Cloud Storage.
bq_table_save(
  bq_dataset_query(Sys.getenv("WORKSPACE_CDR"), dataset_68236488_device_sql, billing = Sys.getenv("GOOGLE_PROJECT")),
  device_68236488_path,
  destination_format = "CSV")


# Read the data directly from Cloud Storage into memory.
# NOTE: Alternatively you can `gsutil -m cp {device_68236488_path}` to copy these files
#       to the Jupyter disk.
read_bq_export_from_workspace_bucket <- function(export_path) {
...
dataset_68236488_device_df <- read_bq_export_from_workspace_bucket(device_68236488_path)

dim(dataset_68236488_device_df)

head(dataset_68236488_device_df, 5)
library(tidyverse)
library(bigrquery)

# This query represents dataset "cohort data" for domain "drug" and was generated for All of Us Controlled Tier Dataset v7
dataset_68236488_drug_sql <- paste("
    SELECT
...
drug_68236488_path <- file.path(
  ...
  "drug_68236488",
  "drug_68236488_*.csv")
message(str_glue('The data will be written to {drug_68236488_path}. Use this path when reading ',
                 'the data into your notebooks in the future.'))
...
#       just read data from the CSVs in Cloud Storage.
bq_table_save(
  bq_dataset_query(Sys.getenv("WORKSPACE_CDR"), dataset_68236488_drug_sql, billing = Sys.getenv("GOOGLE_PROJECT")),
  drug_68236488_path,
  destination_format = "CSV")


# Read the data directly from Cloud Storage into memory.
# NOTE: Alternatively you can `gsutil -m cp {drug_68236488_path}` to copy these files
#       to the Jupyter disk.
read_bq_export_from_workspace_bucket <- function(export_path) {
...
dataset_68236488_drug_df <- read_bq_export_from_workspace_bucket(drug_68236488_path)

dim(dataset_68236488_drug_df)

head(dataset_68236488_drug_df, 5)
```
...
```{r}
# rename datasets
condition_df <- dataset_68236488_condition_df
device_df <- dataset_68236488_device_df
drug_df <- dataset_68236488_drug_df
measurement_df <- dataset_68236488_measurement_df
observation_df <- dataset_68236488_observation_df
person_df <- dataset_68236488_person_df
procedure_df <- dataset_68236488_procedure_df
...
# condition_df <- add_race_column(condition_df, person_df)
# device_df <- add_race_column(device_df, person_df)
# drug_df <- add_race_column(drug_df, person_df)
# measurement_df <- add_race_column(measurement_df, person_df)
# observation_df <- add_race_column(observation_df, person_df)
# procedure_df <- add_race_column(procedure_df, person_df)
...
##########################################################################
# person
# Select the specified columns from the person dataset
# person_df <- person_df %>%
#   select(person_id, date_of_birth, race, sex_at_birth, ethnicity)
# 
# # Group by person_id and summarize the count of each
# person_summary <- person_df %>%
#   group_by(person_id) %>%
#   summarize(count = n())
# 
...
icd_code <- list(
  ...
  c("I70.2", "440.20"), #12.1
  c("I70.1", "440.1"), #12.2
  c("I70.26", "440.24"), #12.3
  c("R78.81", "790.7"), #13
  c("R23.4", "782.0"), #14
  c("N03", "582"), #15.1
  c("N00.9", "580.9"), #15.2 #same as #5 above
  c("D89.81", "279.50"), #16
  c("N18.1", "585.1"), #17
  c("N18.2", "585.2"), #18
  c("N18.3", "585.3"), #19
  ...
  c("G89.4", "338.29"), #21
  c("N18.5", "585.5"), #22
  c("L97.919", "L97.929", "707.10"), #23
  c("K55.1", "569.89"), #24
  c("A04.71", "A04.72", "008.45"), #25
  
  c("Z96.41", "V45.85"), #26
  c("T85.71", "T85.71XA", "Z49.02", "T85.71XD", "T85.71XS", "Z49.32", "996.56", "996.68", "V56.2", "V56.32" ), #27 
  c("Z49.0", "Z99.2", "V45.11"), #28.1 #If 27 or 28.2 is true, make true
  #c("Z99.2", "V45.11"), #28.2
  #c("Q60", "753"), #29
  #c("Q78.6", "756"), #30
  c("I50.9", "428.0"), #31
  ...
  c("I77.1", "447.1"), #35
  c("I99.9", "459.9"), #36
  c("E10.3", "250.51", "250.53"), #37
  c("E11.3", "250.50", "250.52"), #38
  c("K03.6", "521.00"), #39
  c("N28.8", "593.89"), #40
  c("E83.9", "275"), #41
  c("M62.81", "359.8"), #42
  c("E21", "252"), #43
  c("N48.9", "607.84"), #44
  c("E83.3" ,"E83.59", "275.3"), #45
  c("E83.39", "275.3"), #46 #partially included in 45 above
  c("E88.1", "277.3"), #47
  c("E80.2", "277.1"), #48
  c("E72.2", "270.6"), #49
  c("N18.6", "585.6"), #50
...
filtered_rows <- condition_df %>%
  filter(
    source_concept_code %in% c("29738008", "R80", "791.0") |
...
Without Roll-up Count, select on only SNOMED

```{r}
# # Create a dataframe to map conditions and SNOMED codes
...
# 
# # Loop through each condition and filter the dataset based on SNOMED codes
# for (i in 1:nrow(conditions_df)) {
#   condition_name <- conditions_df$condition[i]
#   snomed_codes_list <- unlist(strsplit(as.character(conditions_df$snomed_code[i]), ", "))
# 
#   temp_df <- condition %>%
#     filter(standard_concept_code %in% snomed_codes_list) %>%
#     mutate(new_name = condition_name)
# 
...
Without Roll-up Count, select on only ICD

```{r}
# # Create a dataframe to map conditions, SNOMED, and ICD codes
# conditions_df <- data.frame(
#   condition = conditions,
#   icd_code = I(sapply(icd_codes, function(x) toString(x)))
# )
# 
# # Initialize an empty dataframe to store the results
# filtered_conditions <- data.frame()
# 
# # Loop through each condition and filter the dataset based on ICD codes
# for (i in 1:nrow(conditions_df)) {
#   condition_name <- conditions_df$condition[i]
#   icd_codes_list <- unlist(strsplit(as.character(conditions_df$icd_code[i]), ", "))
# 
#   temp_df <- condition %>%
#     filter(rowSums(sapply(icd_codes_list, function(x) grepl(paste0("^", x), source_concept_code))) > 0) %>%
#     mutate(new_name = condition_name)
# 
#   filtered_conditions <- rbind(filtered_conditions, temp_df)
# }
...
conditions_df <- data.frame(
  ...
  phecode = phecode,
  snomed_code = I(sapply(snomed_codes, function(x) toString(x))),
  icd_code = I(sapply(icd_code, function(x) toString(x)))
)
...
for (i in 1:nrow(conditions_df)) {
  ...
  temp_df <- condition_df %>%
    filter(standard_concept_code %in% snomed_code | source_concept_code %in% icd_codes_list) %>%
...
With Roll-up count select on both SNOMED or ICD
...
#   
#   temp_df <- condition_df %>%
#     filter(standard_concept_code %in% snomed_code | rowSums(sapply(icd_codes_list, function(x) grepl(paste0("^", x), source_concept_code))) > 0) %>%
#     mutate(new_name = phecode_name)
#   
#   filtered_conditions <- rbind(filtered_conditions, temp_df)
# }
...
```{r}
# # Identify the duplicated rows based on specified columns
# duplicates <- filtered_conditions %>%
#   group_by(person_id, new_name, visit_occurrence_id, condition_start_datetime, condition_end_datetime) %>%
#   filter(n() > 1) %>%
#   ungroup()
# 
# # Extract unique rows that correspond to the identified duplicates
# unique_rows <- filtered_conditions %>%
#   filter(paste(person_id, new_name, visit_occurrence_id, condition_start_datetime, condition_end_datetime) %in%
#            paste(duplicates$person_id, duplicates$new_name, duplicates$visit_occurrence_id, duplicates$condition_start_datetime, duplicates$condition_end_datetime)) %>%
#   distinct()
# 
# # Combine the duplicated rows and the corresponding unique rows for comparison
# comparison_df <- unique_rows %>%
#   left_join(duplicates, by = c("person_id", "new_name", "visit_occurrence_id", "condition_start_datetime", "condition_end_datetime"),
#             suffix = c(".original", ".duplicate"))
# 
# # Print the comparison data frame
# print("Comparison of original and duplicated rows:")
# print(comparison_df)
...
#   ungroup()
# 
# # Remove all duplicated rows from filtered_conditions
# filtered_conditions <- filtered_conditions %>%
#   anti_join(duplicates, by = c("person_id", "new_name", "visit_occurrence_id", "condition_start_datetime", "condition_end_datetime"))
# 
# # Check to confirm there are no duplicates left
# no_duplicates_check <- filtered_conditions %>%
#   group_by(person_id, new_name, visit_occurrence_id, condition_start_datetime, condition_end_datetime) %>%
#   filter(n() > 1) %>%
#   ungroup()
# 
# # Print the result to confirm
# print("Rows with duplicates after removal:")
# print(no_duplicates_check)
# 
...
measurements <- list(
  ...
  "Creatinine [Mass/volume] in Body fluid", #5
...
```{r}
# Step 1: Filter the measurement data for Body height and Body weight
body_measurements <- measurement_df %>%
  filter(standard_concept_name %in% c("Body height", "Body weight"))

# Step 2: Reshape the data to have Body height and Body weight in separate columns
...
# Step 3: Replace non-positive values with NA
body_measurements_wide <- body_measurements_wide %>%
  mutate(
    `Body height` = ifelse(`Body height` > 0, `Body height`, NA),
    `Body weight` = ifelse(`Body weight` > 0, `Body weight`, NA)
  )

# Step 4: Calculate BMI for each person_id
bmi_table <- body_measurements_wide %>%
  mutate(
    BMI = ifelse(!is.na(`Body height`) & !is.na(`Body weight`), `Body weight` / ((`Body height` / 100) ^ 2), NA)
  ) %>%
  select(person_id, BMI)

# Step 5: Join the BMI data back to the person dataset
person <- person %>%
  left_join(bmi_table, by = "person_id")
...
# # Group by standard_concept_name and summarize the count of each
# procedure_summary <- procedure_df %>%
#   group_by(standard_concept_name) %>%
#   summarize(count = n()) %>%
#   arrange(desc(count))
# 
...
procedure_source_code <- list(
  c(39.42, 36821), #1
  c("5A1D00Z", 39.95, 90935), #2
  c(55.4, 55.5, "0TB00ZX", "0TT00ZZ", 50240, 50220), #3
  c(54.98, "3E1M39Z", 90945), #4
  "G0299" #5
)


# Determine the maximum length among all lists
max_length <- max(length(procedure_list), max(lengths(procedure_codes)), max(lengths(procedure_source_code)))

# Extend lists to match the maximum length
procedure_list <- rep(procedure_list, length.out = max_length)
procedure_codes <- unlist(lapply(procedure_codes, function(x) rep(x, length.out = max_length)))
procedure_source_code <- unlist(lapply(procedure_source_code, function(x) rep(x, length.out = max_length)))

# Create the data frame
procedures_df <- data.frame(
  procedure = procedure_list,
  procedure_code = as.character(procedure_codes),  # Convert to character
  procedure_source_code = as.character(procedure_source_code),  # Convert to character
  stringsAsFactors = FALSE
)


# Convert standard_concept_code and source_concept_code to character
procedure_df <- procedure_df %>%
  mutate(
    standard_concept_code = as.character(standard_concept_code),
    source_concept_code = as.character(source_concept_code)
  )
...
for (i in 1:nrow(procedures_df)) {
  ...
  procedure_source_code_list <- unlist(strsplit(as.character(procedures_df$procedure_source_code[i]), ", "))

  temp_df <- procedure_df %>%
    filter(standard_concept_code %in% procedure_code | source_concept_code %in% procedure_source_code_list) %>%
...
first_measurements <- first_measurements %>%
  mutate(group = case_when(
    value_as_number > 500 ~ ">500",
    value_as_number > 120 & value_as_number <= 500 ~ "121-500",
    value_as_number >= 90 & value_as_number <= 120 ~ "90-120",
    value_as_number >= 60 & value_as_number < 90 ~ "60-89",
    value_as_number >= 45 & value_as_number < 60 ~ "45-59",
    value_as_number >= 30 & value_as_number < 45 ~ "30-44",
    value_as_number >= 15 & value_as_number < 30 ~ "15-29",
    value_as_number < 15 ~ "<15",
...
Inderterminate eGFR value

```{r}
# # Filter rows where first_eGFR_value is NA
# rows_with_na_first_eGFR_value <- person %>%
#   filter(is.na(first_eGFR_value))
# 
# # Print the rows with NA first_eGFR_value
# print(rows_with_na_first_eGFR_value)
# 
# 
# # Filter out rows where first_eGFR_value is NA
# person_NA <- person %>%
#   filter(!is.na(first_eGFR_value))
...
first_measurements <- first_measurements %>%
  ...
  slice(1) %>%
  ungroup() %>%
  select(person_id, measurement_datetime, measurement_type_concept_name, value_as_number, group) %>%
  rename(index_date = measurement_datetime, eGFR_value = value_as_number, eGFR_group = group, eGFR_measurement_type = measurement_type_concept_name)

# Add the index_date, eGFR_value, and eGFR_group to the person dataset
person <- person %>%
...
```{r}
# # Join 'index_date' from 'person' to 'filtered_conditions'
...
#   pivot_wider(names_from = feature_window, values_from = unique_person_count)
# 
...
# ml_data <- filtered_person_with_summary
# 
...
data <- read.csv("filtered_cohort_data.csv")
colnames(data)
```
...
km_fit <- survfit(Surv(time_to_event, event) ~ combined_group, data = person_with_summary)
...
inrange_person_with_summary <- read.csv("inrange_person_with_summary.csv")
...
plausible_acr_min <- 0
plausible_acr_max <- 8000  # 8000 mg/g is a very high value, but still clinically plausible, confirm units!!

# Filter out implausible eGFR and Albumin/Creatinine values
person_filtered_KFRE <- inrange_person_with_summary %>%
  select(person_id, sex_at_birth, eGFR_value, Albumin_Creatinine = `Albumin.Creatinine`, age_precise, race, disease_status, time_to_event, )
...
person_filtered_KFRE <- person_filtered_KFRE %>%
  ...
  mutate(logACR = log(Albumin_Creatinine + 1)) %>%
...
# Create 2-year, 5-year outcomes, and beyond 5-year outcomes based on disease status and time to event
consistent_data <- consistent_data %>%
  mutate(
    ...
    outcome_beyond_5_years = ifelse(disease_status == 1 & time_to_event > 5 * 365.25, 1, 0)
  )

# Define a function to calculate AUC, Harrell's C-statistic, and Somers' D
calculate_metrics <- function(data, risk_col, outcome_col, time_col) {
  ...
  return(list(AUC = auc_value, C_Statistic = c_stat, Somers_D = somers_d))
}

# Define a function to summarize the risk metrics, AUC, Harrell's C-statistic, and Somers' D for each subset
summarize_risk_metrics <- function(data, race_label) {
  ...
  metrics_2_years_val <- calculate_metrics(validation_data, "risk_2_years", "outcome_2_years", "time_to_event")
  metrics_5_years_val <- calculate_metrics(validation_data, "risk_5_years", "outcome_5_years", "time_to_event")

  train_case_control_2yr <- table(train_data$outcome_2_years)
  validation_case_control_2yr <- table(validation_data$outcome_2_years)
  train_case_control_5yr <- table(train_data$outcome_5_years)
  validation_case_control_5yr <- table(validation_data$outcome_5_years)

  data %>%
    summarise(
      `Race` = race_label,
      `AUC 2 Years Validation` = metrics_2_years_val$AUC,
      `Harrell's C-Statistic 2 Years Validation` = metrics_2_years_val$C_Statistic,
      `AUC 5 Years Validation` = metrics_5_years_val$AUC,
      `Harrell's C-Statistic 5 Years Validation` = metrics_5_years_val$C_Statistic,
      `Train Cases 2 Years` = train_case_control_2yr["1"],
      `Train Controls 2 Years` = train_case_control_2yr["0"],
      `Validation Cases 2 Years` = validation_case_control_2yr["1"],
      `Validation Controls 2 Years` = validation_case_control_2yr["0"],
      `Train Cases 5 Years` = train_case_control_5yr["1"],
      `Train Controls 5 Years` = train_case_control_5yr["0"],
      `Validation Cases 5 Years` = validation_case_control_5yr["1"],
      `Validation Controls 5 Years` = validation_case_control_5yr["0"]
    )
}
...
cat("\nCombined Risk Metrics, AUC, Harrell's C-Statistic, and Case-Control Counts for Different Races:\n")
print(final_results)
...
## Logistic regression and Elastic net model
...
# Function to run the analysis and return results
run_analysis <- function(data, race_group) {
  # Step 1: Data preprocessing
  data_filtered <- data %>%
    select(person_id, sex_at_birth, eGFR_value, Albumin_Creatinine = `Albumin.Creatinine`, age_precise, race, disease_status, time_to_event) %>%
    filter(
      eGFR_value >= 0 & eGFR_value <= 60,
      Albumin_Creatinine >= 0 & Albumin_Creatinine <= 8000,
      sex_at_birth %in% c("Female", "Male")
    ) %>%
    mutate(
      logACR = log(Albumin_Creatinine + 1),
      outcome_2_years = ifelse(time_to_event <= 2 * 365.25 & disease_status == 1, 1, 0),
      outcome_5_years = ifelse(time_to_event <= 5 * 365.25 & disease_status == 1, 1, 0)
    ) %>%
    filter(complete.cases(.))
  
  # Filter by race group if specified
  if (!is.null(race_group)) {
    data_filtered <- data_filtered %>% filter(race == race_group)
  }

  # Step 2: Split the data into training (70%) and validation (30%) sets
  set.seed(42)  # For reproducibility
  train_indices <- sample(seq_len(nrow(data_filtered)), size = 0.7 * nrow(data_filtered))
  train_data <- data_filtered[train_indices, ]
  validation_data <- data_filtered[-train_indices, ]

  # Count the number of cases and controls for 2-year and 5-year outcomes
  train_case_control_2yr <- table(train_data$outcome_2_years)
  validation_case_control_2yr <- table(validation_data$outcome_2_years)
  train_case_control_5yr <- table(train_data$outcome_5_years)
  validation_case_control_5yr <- table(validation_data$outcome_5_years)

  # Step 3: Define the formula for βsum
  calculate_beta_sum <- function(age, sex, eGFR, logACR, race) {
    -0.2201 * (age / 10 - 7.036) +
    0.2467 * (ifelse(sex == "Male", 1, 0) - 0.5642) -
    0.5567 * (eGFR / 5 - 7.222) +
    0.4510 * (logACR - 5.137) +
    log(2.65) * (ifelse(race == "Black or African American", 1, 0))  # Adjusted coefficient for race
  }

  # Step 4: Calculate βsum for the training set
  train_data <- train_data %>%
    mutate(beta_sum = calculate_beta_sum(age_precise, sex_at_birth, eGFR_value, logACR, race))

  # Step 5: Logistic regression calibration for 2-year and 5-year risk
  calibration_model_2yr <- glm(outcome_2_years ~ beta_sum, data = train_data, family = "binomial")
  calibration_model_5yr <- glm(outcome_5_years ~ beta_sum, data = train_data, family = "binomial")

  # Step 6: Calculate βsum and apply calibration to the validation set
  validation_data <- validation_data %>%
    mutate(beta_sum = calculate_beta_sum(age_precise, sex_at_birth, eGFR_value, logACR, race)) %>%
    mutate(
      calibrated_prob_2yr = predict(calibration_model_2yr, newdata = ., type = "response"),
      calibrated_prob_5yr = predict(calibration_model_5yr, newdata = ., type = "response")
    ) %>%
    filter(is.finite(calibrated_prob_2yr) & is.finite(calibrated_prob_5yr))

  # Step 7: Evaluate the calibration model on the validation set for both 2-year and 5-year risk
  roc_curve_2yr <- roc(validation_data$outcome_2_years, validation_data$calibrated_prob_2yr)
  roc_curve_5yr <- roc(validation_data$outcome_5_years, validation_data$calibrated_prob_5yr)
  auc_value_2yr <- auc(roc_curve_2yr)
  auc_value_5yr <- auc(roc_curve_5yr)
  
  # Step 8: Regularization (Elastic Net) model for 2-year and 5-year risk
  
  # Check for single-level categorical variables
  if(length(unique(train_data$sex_at_birth)) > 1 & length(unique(train_data$race)) > 1) {
    X_train <- model.matrix(~ age_precise + sex_at_birth + eGFR_value + logACR + race, data = train_data)[,-1]  # Exclude intercept
    X_validation <- model.matrix(~ age_precise + sex_at_birth + eGFR_value + logACR + race, data = validation_data)[,-1]  # Exclude intercept
  } else {
    # If any variable has only one level, exclude it from the model matrix
    X_train <- model.matrix(~ age_precise + eGFR_value + logACR, data = train_data)[,-1]  # Exclude intercept
    X_validation <- model.matrix(~ age_precise + eGFR_value + logACR, data = validation_data)[,-1]  # Exclude intercept
  }
  
  y_train_2yr <- as.numeric(train_data$outcome_2_years)  # 2-year outcome
  y_train_5yr <- as.numeric(train_data$outcome_5_years)  # 5-year outcome

  # Fit Elastic Net model for 2-year risk
  elastic_net_model_2yr <- cv.glmnet(X_train, y_train_2yr, family = "binomial", alpha = 0.5)
  best_lambda_2yr <- elastic_net_model_2yr$lambda.min

  # Fit Elastic Net model for 5-year risk
  elastic_net_model_5yr <- cv.glmnet(X_train, y_train_5yr, family = "binomial", alpha = 0.5)
  best_lambda_5yr <- elastic_net_model_5yr$lambda.min

  # Apply to validation set for 2-year risk
  predicted_probabilities_2yr <- as.vector(predict(elastic_net_model_2yr, s = best_lambda_2yr, newx = X_validation, type = "response"))

  # Apply to validation set for 5-year risk
  predicted_probabilities_5yr <- as.vector(predict(elastic_net_model_5yr, s = best_lambda_5yr, newx = X_validation, type = "response"))

  # Evaluate the elastic net model
  roc_curve_en_2yr <- roc(validation_data$outcome_2_years, predicted_probabilities_2yr)  # 2-year outcome
  roc_curve_en_5yr <- roc(validation_data$outcome_5_years, predicted_probabilities_5yr)  # 5-year outcome
  auc_value_en_2yr <- auc(roc_curve_en_2yr)
  auc_value_en_5yr <- auc(roc_curve_en_5yr)

  # Return the results as a list
  return(list(
    AUC_2yr = auc_value_2yr,
    AUC_5yr = auc_value_5yr,
    ElasticNet_AUC_2yr = auc_value_en_2yr,
    ElasticNet_AUC_5yr = auc_value_en_5yr,
    Train_Cases_2yr = train_case_control_2yr["1"],
    Train_Controls_2yr = train_case_control_2yr["0"],
    Validation_Cases_2yr = validation_case_control_2yr["1"],
    Validation_Controls_2yr = validation_case_control_2yr["0"],
    Train_Cases_5yr = train_case_control_5yr["1"],
    Train_Controls_5yr = train_case_control_5yr["0"],
    Validation_Cases_5yr = validation_case_control_5yr["1"],
    Validation_Controls_5yr = validation_case_control_5yr["0"]
  ))
}

# Running the analysis for all races, Black or African American, and White groups
all_races_results <- run_analysis(inrange_person_with_summary, race_group = NULL)
black_results <- run_analysis(inrange_person_with_summary, race_group = "Black or African American")
white_results <- run_analysis(inrange_person_with_summary, race_group = "White")

# Compile results into a table
results_table <- data.frame(
  Group = c("All Races", "Black or African American", "White"),
  LogisticReg_AUC_2yr = c(all_races_results$AUC_2yr, black_results$AUC_2yr, white_results$AUC_2yr),
  LogisticReg_AUC_5yr = c(all_races_results$AUC_5yr, black_results$AUC_5yr, white_results$AUC_5yr),
  ElasticNet_AUC_2yr = c(all_races_results$ElasticNet_AUC_2yr, black_results$ElasticNet_AUC_2yr, white_results$ElasticNet_AUC_2yr),
  ElasticNet_AUC_5yr = c(all_races_results$ElasticNet_AUC_5yr, black_results$ElasticNet_AUC_5yr, white_results$ElasticNet_AUC_5yr),
  Train_Cases_2yr = c(all_races_results$Train_Cases_2yr, black_results$Train_Cases_2yr, white_results$Train_Cases_2yr),
  Train_Controls_2yr = c(all_races_results$Train_Controls_2yr, black_results$Train_Controls_2yr, white_results$Train_Controls_2yr),
  Validation_Cases_2yr = c(all_races_results$Validation_Cases_2yr, black_results$Validation_Cases_2yr, white_results$Validation_Cases_2yr),
  Validation_Controls_2yr = c(all_races_results$Validation_Controls_2yr, black_results$Validation_Controls_2yr, white_results$Validation_Controls_2yr),
  Train_Cases_5yr = c(all_races_results$Train_Cases_5yr, black_results$Train_Cases_5yr, white_results$Train_Cases_5yr),
  Train_Controls_5yr = c(all_races_results$Train_Controls_5yr, black_results$Train_Controls_5yr, white_results$Train_Controls_5yr),
  Validation_Cases_5yr = c(all_races_results$Validation_Cases_5yr, black_results$Validation_Cases_5yr, white_results$Validation_Cases_5yr),
  Validation_Controls_5yr = c(all_races_results$Validation_Controls_5yr, black_results$Validation_Controls_5yr, white_results$Validation_Controls_5yr)
)

# Print the results table for publication
print(results_table)
...
person_with_summary <- person_with_summary %>%
  select(-age_reported,
        -`eGFR_measurement_type`, -eGFR_group)
...
# Assuming your dataset is named 'filtered_person_with_summary'
data <- filtered_data


# Step 1: Convert "race" column to numeric (1 for "Black or African American", 0 for "White"),
# and replace NA values in "race" and "ethnicity" with "Not specified"
data <- data %>%
  mutate(race = ifelse(is.na(race), "Not specified", 
                       ifelse(race == "Black or African American", 1,
                              ifelse(race == "White", 0, NA))),
         ethnicity = ifelse(is.na(ethnicity), "Not specified", ethnicity))

# # Step 2: Convert other categorical non-numeric columns to numeric using one-hot encoding
# data <- data %>%
#   mutate(across(where(is.character) & !any_of(c("race", "ethnicity")), as.numeric))

# Step 3: Fill all NA values with 0
data <- data %>%
  mutate(across(everything(), ~ replace_na(., 0)))

# Check the data to ensure all columns are numeric and NA values are filled with 0
#summary(data)

# Step 4: Apply log(value + 1) transformation to all relevant numeric columns
data <- data %>%
  mutate(across(where(is.numeric) & !any_of(c("person_id", "time_to_event", "disease_status", "race", "ethnicity", "Smoking")), ~ log(. + 1)))

# Output column names of the transformed data
#colnames(ml_data_normal)
unique(data$disease_status)


# Convert 'sex_at_birth' and 'ethnicity' to factors (if they are not already)
data <- data %>%
  mutate(sex_at_birth = as.factor(sex_at_birth),
         ethnicity = as.factor(ethnicity),
         Smoking = as.factor(Smoking),
         race = as.factor(race))

# Print reference categories for each factor
cat("Reference category for 'sex_at_birth':", levels(data$sex_at_birth)[1], "\n")
cat("Reference category for 'ethnicity':", levels(data$ethnicity)[1], "\n")
cat("Reference category for 'Smoking':", levels(data$Smoking)[1], "\n")
cat("Reference category for 'race':", levels(data$race)[1], "\n")


# Apply one-hot encoding to 'sex_at_birth' and 'ethnicity'
data <- data %>%
  mutate(across(c(sex_at_birth, ethnicity), ~ as.numeric(as.factor(.))))

# Save the data frame to a CSV file
write.csv(data, "ml_model_data.csv", row.names = FALSE)
...
library(e1071)
library(pROC)
library(smotefamily)  # Alternative to DMwR for SMOTE
library(ggplot2)
...
set.seed(123)
trainIndex <- createDataPartition(data$outcome_2_year, p = 0.7, list = FALSE)
...
# Step 4: Convert data to matrix format for XGBoost
...
# Convert labels to factors for classification with valid levels
train_label_2_year <- factor(train_data$outcome_2_year, levels = c(0, 1), labels = c("class0", "class1"))
train_label_5_year <- factor(train_data$outcome_5_year, levels = c(0, 1), labels = c("class0", "class1"))

test_label_2_year <- factor(test_data$outcome_2_year, levels = c(0, 1), labels = c("class0", "class1"))
test_label_5_year <- factor(test_data$outcome_5_year, levels = c(0, 1), labels = c("class0", "class1"))

# Step 5: Calculate class weights based on the 2-year and 5-year training data
class_weights_2_year <- table(train_label_2_year)
class_weights_2_year <- max(class_weights_2_year) / class_weights_2_year

class_weights_5_year <- table(train_label_5_year)
class_weights_5_year <- max(class_weights_5_year) / class_weights_5_year

# Common XGBoost parameter grid
...
# Function to count cases and controls with explicit handling of missing labels
...
# Calculate counts for the 2-year and 5-year training and test sets
...
# Count cases and controls in the training and test set stratified by race
...
# Handle race counts for missing combinations
...
summary_2_year <- bind_rows(
  data.frame(Set = "Training Set", Outcome = "2-Year", Race = "Overall", Cases = train_2_year_counts$count[train_2_year_counts$label == "Cases"], Controls = train_2_year_counts$count[train_2_year_counts$label == "Controls"]),
  train_2_year_race_combined %>%
    mutate(Set = "Training Set", Outcome = "2-Year", Race = ifelse(race == 1, "Black or African American", "White")) %>%
    select(Set, Outcome, Race, Cases, Controls),
  data.frame(Set = "Test Set", Outcome = "2-Year", Race = "Overall", Cases = test_2_year_counts$count[test_2_year_counts$label == "Cases"], Controls = test_2_year_counts$count[test_2_year_counts$label == "Controls"]),
  test_2_year_race_combined %>%
    mutate(Set = "Test Set", Outcome = "2-Year", Race = ifelse(race == 1, "Black or African American", "White")) %>%
    select(Set, Outcome, Race, Cases, Controls)
)

# Summary for 5-Year
summary_5_year <- bind_rows(
  data.frame(Set = "Training Set", Outcome = "5-Year", Race = "Overall", Cases = train_5_year_counts$count[train_5_year_counts$label == "Cases"], Controls = train_5_year_counts$count[train_5_year_counts$label == "Controls"]),
  train_5_year_race_combined %>%
    mutate(Set = "Training Set", Outcome = "5-Year", Race = ifelse(race == 1, "Black or African American", "White")) %>%
    select(Set, Outcome, Race, Cases, Controls),
  data.frame(Set = "Test Set", Outcome = "5-Year", Race = "Overall", Cases = test_5_year_counts$count[test_5_year_counts$label == "Cases"], Controls = test_5_year_counts$count[test_5_year_counts$label == "Controls"]),
  test_5_year_race_combined %>%
    mutate(Set = "Test Set", Outcome = "5-Year", Race = ifelse(race == 1, "Black or African American", "White")) %>%
    select(Set, Outcome, Race, Cases, Controls)
)
...
### 0. **Data Preparation and Common Setup**
...
# Step 4: Convert labels to factors for classification with valid levels
...
### 1. **XGBoost Model for 2-Year Prediction**
...
# Set fixed hyperparameters for XGBoost model
xgb_fixed_params <- list(
  nrounds = 100,  # Fixed number of rounds
  max_depth = 3,  # Fixed max depth
  eta = 0.1,  # Fixed learning rate
  gamma = 0,  # No gamma
  colsample_bytree = 0.7,  # Fixed sampling
  min_child_weight = 1,  # Fixed min_child_weight
  subsample = 0.8  # Fixed subsample
)

# Ensure train and test matrices are correctly formatted
train_matrix_2_year <- as.matrix(train_matrix_2_year)
test_matrix_2_year <- as.matrix(test_matrix_2_year)

# Define the race vectors from your dataset
train_race_vector <- train_data$race
test_race_vector <- test_data$race

# Function to run XGBoost and capture performance metrics
run_xgb_model <- function(train_matrix, test_matrix, train_labels, test_labels, class_weights) {
  xgb_fixed_params <- list(
    nrounds = 100,  # Fixed number of rounds
    max_depth = 3,  # Fixed max depth
    eta = 0.1,  # Fixed learning rate
  ...
  xgb_model <- train(
    ...
    trControl = trainControl(
      method = "none",  # Use a single train-test split for faster execution
      classProbs = TRUE,
      summaryFunction = twoClassSummary,
      verboseIter = TRUE  # To monitor the progress
    ),
    tuneGrid = data.frame(xgb_fixed_params),
    metric = "ROC",
    scale_pos_weight = class_weights[2] / class_weights[1]
  )

  # Predict on the test set
  xgb_probs <- predict(xgb_model, newdata = test_matrix, type = "prob")[, 2]
  xgb_preds <- ifelse(xgb_probs > 0.5, "class1", "class0")

  # Evaluate model performance using AUC
  auc <- roc(test_labels, xgb_probs)$auc

  # Calculate the confusion matrix
  conf_matrix <- confusionMatrix(as.factor(xgb_preds), as.factor(test_labels), positive = "class1")
  ...
  importance_scores <- xgb.importance(feature_names = colnames(train_matrix), model = xgb_model$finalModel)

  # Remove any reference to 2-year outcomes from the importance table if necessary
  importance_scores <- importance_scores %>%
    filter(!Feature %in% c("outcome_5_yearclass0", "outcome_5_yearclass1"))

  # Print the filtered variable importance table
  print(importance_scores)

  # Return performance metrics and importance scores
  return(list(
    metrics = data.frame(AUC = auc, Sensitivity = sensitivity, Specificity = specificity, Accuracy = accuracy),
    importance_scores = importance_scores
  ))
}

# Define the overall performance and importance storage
combined_performance <- data.frame()
all_importance_scores <- list()

# Run the model for all individuals
result_all <- run_xgb_model(train_matrix_2_year, test_matrix_2_year, train_label_2_year, test_label_2_year, class_weights_2_year)
combined_performance <- rbind(combined_performance, cbind(Race = "All", result_all$metrics))
all_importance_scores[["All"]] <- result_all$importance_scores

# Run the model for individuals with race == 1 (Black or African American)
train_matrix_race_1 <- train_matrix_2_year[train_race_vector == 1, ]
test_matrix_race_1 <- test_matrix_2_year[test_race_vector == 1, ]
train_label_race_1 <- train_label_2_year[train_race_vector == 1]
test_label_race_1 <- test_label_2_year[test_race_vector == 1]
class_weights_race_1 <- table(train_label_race_1)
result_race_1 <- run_xgb_model(train_matrix_race_1, test_matrix_race_1, train_label_race_1, test_label_race_1, class_weights_race_1)
combined_performance <- rbind(combined_performance, cbind(Race = "Black or African American", result_race_1$metrics))
all_importance_scores[["Black or African American"]] <- result_race_1$importance_scores

# Run the model for individuals with race == 0 (White)
train_matrix_race_0 <- train_matrix_2_year[train_race_vector == 0, ]
test_matrix_race_0 <- test_matrix_2_year[test_race_vector == 0, ]
train_label_race_0 <- train_label_2_year[train_race_vector == 0]
test_label_race_0 <- test_label_2_year[test_race_vector == 0]
class_weights_race_0 <- table(train_label_race_0)
result_race_0 <- run_xgb_model(train_matrix_race_0, test_matrix_race_0, train_label_race_0, test_label_race_0, class_weights_race_0)
combined_performance <- rbind(combined_performance, cbind(Race = "White", result_race_0$metrics))
all_importance_scores[["White"]] <- result_race_0$importance_scores
...
write.csv(combined_performance, "xgb_2_year_performance_by_race.csv", row.names = FALSE)

# Save the importance scores for each group
write.csv(all_importance_scores[["All"]], "xgb_2_year_importance_scores_all.csv", row.names = FALSE)
write.csv(all_importance_scores[["Black or African American"]], "xgb_2_year_importance_scores_black_or_african_american.csv", row.names = FALSE)
write.csv(all_importance_scores[["White"]], "xgb_2_year_importance_scores_white.csv", row.names = FALSE)
...
### 2. **Random Forest Model for 2-Year Prediction**
...
# Function to run the Random Forest model and capture performance metrics
run_rf_model_by_race <- function(train_data, test_data, race_value = NULL, label_column, outcome_column) {
  
  # Filter data based on race if specified
  if (!is.null(race_value)) {
    train_subset <- train_data %>% filter(race == race_value)
    test_subset <- test_data %>% filter(race == race_value)
  } else {
  ...
  rf_model <- randomForest(
    ...
    data = train_subset %>% select(-race),
    ntree = 500,
    mtry = floor(sqrt(ncol(train_subset) - 2))  # Exclude race column
  )
  
  # Predictions (probabilities and classes)
  rf_probs <- predict(rf_model, newdata = test_subset %>% select(-race), type = "prob")[,2]
  ...
  race_label <- ifelse(is.null(race_value), "all_races", ifelse(race_value == 1, "black_african_american", "white"))
  ...
  write.csv(var_importance, paste0("rf_2_year_var_importance_", race_label, ".csv"), row.names = FALSE)
  
  # Save the plot
  ggsave(paste0("rf_2_year_var_importance_plot_", race_label, ".png"), plot = importance_plot)
...
# Assume train_data and test_data are already defined, and "race", "outcome_2_year" columns are present

# Run the model for all races
result_all <- run_rf_model_by_race(train_data, test_data, label_column = "outcome_2_year", outcome_column = "outcome_2_year")

# Run the model for race == 1 (Black or African American)
result_race_1 <- run_rf_model_by_race(train_data, test_data, race_value = 1, label_column = "outcome_2_year", outcome_column = "outcome_2_year")

# Run the model for race == 0 (White)
result_race_0 <- run_rf_model_by_race(train_data, test_data, race_value = 0, label_column = "outcome_2_year", outcome_column = "outcome_2_year")

# Combine performance results into a single table
combined_performance <- bind_rows(result_all, result_race_1, result_race_0)
...
# Save the combined performance table
write.csv(combined_performance, "rf_2_year_performance_combined.csv", row.names = FALSE)
...
run_glmnet_model_by_race <- function(train_matrix, test_matrix, train_labels, test_labels, train_race_vector, test_race_vector, race_value = NULL) {
  ...
  glmnet_grid <- expand.grid(
    alpha = c(0, 0.5, 1),  # 0 for ridge, 1 for lasso, 0.5 for elastic net
...
### 1. **XGBoost Model for 5-Year Prediction**
...
# Ensure train and test matrices are correctly formatted
train_matrix_5_year <- as.matrix(train_matrix_5_year)
test_matrix_5_year <- as.matrix(test_matrix_5_year)
...
run_xgb_model <- function(train_matrix, test_matrix, train_labels, test_labels, class_weights) {
  ...
  # Remove any reference to 2-year outcomes from the importance table if necessary
  importance_scores <- importance_scores %>%
    filter(!Feature %in% c("outcome_2_yearclass0", "outcome_2_yearclass1"))
...
# Run the model for all individuals
result_all <- run_xgb_model(train_matrix_5_year, test_matrix_5_year, train_label_5_year, test_label_5_year, class_weights_5_year)
combined_performance <- rbind(combined_performance, cbind(Race = "All", result_all$metrics))
all_importance_scores[["All"]] <- result_all$importance_scores

# Run the model for individuals with race == 1 (Black or African American)
train_matrix_race_1 <- train_matrix_5_year[train_race_vector == 1, ]
test_matrix_race_1 <- test_matrix_5_year[test_race_vector == 1, ]
train_label_race_1 <- train_label_5_year[train_race_vector == 1]
test_label_race_1 <- test_label_5_year[test_race_vector == 1]
class_weights_race_1 <- table(train_label_race_1)
result_race_1 <- run_xgb_model(train_matrix_race_1, test_matrix_race_1, train_label_race_1, test_label_race_1, class_weights_race_1)
combined_performance <- rbind(combined_performance, cbind(Race = "Black or African American", result_race_1$metrics))
all_importance_scores[["Black or African American"]] <- result_race_1$importance_scores
...
# Run the model for individuals with race == 0 (White)
train_matrix_race_0 <- train_matrix_5_year[train_race_vector == 0, ]
test_matrix_race_0 <- test_matrix_5_year[test_race_vector == 0, ]
train_label_race_0 <- train_label_5_year[train_race_vector == 0]
test_label_race_0 <- test_label_5_year[test_race_vector == 0]
class_weights_race_0 <- table(train_label_race_0)
result_race_0 <- run_xgb_model(train_matrix_race_0, test_matrix_race_0, train_label_race_0, test_label_race_0, class_weights_race_0)
combined_performance <- rbind(combined_performance, cbind(Race = "White", result_race_0$metrics))
all_importance_scores[["White"]] <- result_race_0$importance_scores
...
# Save the combined performance table
write.csv(combined_performance, "xgb_5_year_performance_by_race.csv", row.names = FALSE)

# Save the importance scores for each group
write.csv(all_importance_scores[["All"]], "xgb_5_year_importance_scores_all.csv", row.names = FALSE)
write.csv(all_importance_scores[["Black or African American"]], "xgb_5_year_importance_scores_black_or_african_american.csv", row.names = FALSE)
write.csv(all_importance_scores[["White"]], "xgb_5_year_importance_scores_white.csv", row.names = FALSE)
...
### 2. **Random Forest Model for 5-Year Prediction**
...
run_rf_model_by_race <- function(train_data, test_data, race_value = NULL, label_column, outcome_column) {
  ...
  # Save variable importance table
  write.csv(var_importance, paste0("rf_5_year_var_importance_", race_label, ".csv"), row.names = FALSE)
  
  # Save the plot
  ggsave(paste0("rf_5_year_var_importance_plot_", race_label, ".png"), plot = importance_plot)
...
# Define your data for training and testing
# Assume train_data and test_data are already defined, and "race", "outcome_5_year" columns are present

# Run the model for all races
result_all <- run_rf_model_by_race(train_data, test_data, label_column = "outcome_5_year", outcome_column = "outcome_5_year")

# Run the model for race == 1 (Black or African American)
result_race_1 <- run_rf_model_by_race(train_data, test_data, race_value = 1, label_column = "outcome_5_year", outcome_column = "outcome_5_year")

# Run the model for race == 0 (White)
result_race_0 <- run_rf_model_by_race(train_data, test_data, race_value = 0, label_column = "outcome_5_year", outcome_column = "outcome_5_year")
...
# Save the combined performance table
write.csv(combined_performance, "rf_5_year_performance_combined.csv", row.names = FALSE)
...
### 3. **GLMNET Model for 5-Year Prediction**
...
# Step 2: Convert time_to_event thresholds from years to days and create 2-year and 5-year outcome variables
data <- data %>%
  mutate(
    outcome_2_year = factor(ifelse(time_to_event <= 730 & disease_status == 1, 1, 0), levels = c(0, 1), labels = c("class0", "class1")),  # 730 days = 2 years
    outcome_5_year = factor(ifelse(time_to_event <= 1825 & disease_status == 1, 1, 0), levels = c(0, 1), labels = c("class0", "class1"))  # 1825 days = 5 years
...
# Step 3: Split the data into training and testing sets (70/30)
set.seed(123)
trainIndex <- createDataPartition(data$outcome_5_year, p = 0.7, list = FALSE)
train_data <- data[trainIndex, ]
test_data <- data[-trainIndex, ]

# Step 4: Exclude non-covariate columns
train_data <- train_data %>%
  select(-c(person_id, time_to_event, disease_status))
...
# Step 6: Calculate class weights based on the 5-year training data
class_weights_5_year <- table(train_label_5_year)
class_weights_5_year <- max(class_weights_5_year) / class_weights_5_year

# Step 7: Convert data to matrix format for GLMNET
train_matrix_5_year <- model.matrix(outcome_5_year ~ . -1, data = train_data)
test_matrix_5_year <- model.matrix(outcome_5_year ~ . -1, data = test_data)

# Step 8: Extract race vectors
train_race_vector <- train_data$race
test_race_vector <- test_data$race
...
run_glmnet_model_by_race <- function(train_matrix, test_matrix, train_labels, test_labels, train_race_vector, test_race_vector, race_value = NULL) {
  ...
  # Save the coefficients and importance scores
  race_label <- ifelse(is.null(race_value), "all_races", paste0("race_", race_value))
  write.csv(coef_df, paste0("glmnet_5_year_coefficients_", race_label, ".csv"), row.names = FALSE)
  ggsave(paste0("glmnet_5_year_var_importance_plot_", race_label, ".png"), plot = importance_plot)
...
# Run the model for all races
performance_all <- run_glmnet_model_by_race(train_matrix_5_year, test_matrix_5_year, train_label_5_year, test_label_5_year, train_race_vector, test_race_vector)

# Run the model for race == 1 (Black or African American)
performance_race_1 <- run_glmnet_model_by_race(train_matrix_5_year, test_matrix_5_year, train_label_5_year, test_label_5_year, train_race_vector, test_race_vector, race_value = 1)

# Run the model for race == 0 (White)
performance_race_0 <- run_glmnet_model_by_race(train_matrix_5_year, test_matrix_5_year, train_label_5_year, test_label_5_year, train_race_vector, test_race_vector, race_value = 0)
...
# Save the combined performance table
write.csv(combined_performance, "glmnet_5_year_performance_combined.csv", row.names = FALSE)
...
### 4. **SVM Model for 5-Year Prediction**
...
run_svm_model_by_race <- function(train_data, test_data, race_value = NULL) {
  ...
  # Extract labels
  train_label <- train_data$outcome_5_year
  test_label <- test_data$outcome_5_year
  
  # Train the SVM model with class weighting
  svm_model <- train(
    outcome_5_year ~ ., data = train_data %>% select(-race),
    method = "svmRadial",
    trControl = trainControl(
      method = "cv",
      number = 10,
...
# Save the combined performance table
write.csv(combined_performance, "svm_5_year_performance_combined.csv", row.names = FALSE)
...
### 5. **Deep neural network Model for 5-Year Prediction**
...
Tune the regularization parameter λ\lambdaλ.
...
# library(pROC)
# library(ggplot2)
# library(survival)  # For Harrell's C-statistic
# 
# # Start timing the execution
# start_time <- Sys.time()
# 
# # Load dataset
# ml_data_normalized <- read.csv("ml_model_data.csv")
# 
# # Convert disease_status to a factor with two levels: "Class0" and "Class1"
# ml_data_normalized <- ml_data_normalized %>%
#   mutate(disease_status = factor(disease_status, levels = c(0, 1), labels = c("Class0", "Class1")))
# 
# # Prepare the data: Remove person_id and any other non-numeric variables if needed
# ml_data <- ml_data_normalized %>%
#   select(-person_id) %>%
#   filter(complete.cases(.))  # Ensure no missing data
# 
# # Split data into training and testing sets
# set.seed(123)  # For reproducibility
# train_index <- createDataPartition(ml_data$disease_status, p = 0.7, list = FALSE)
# ml_data_train <- ml_data[train_index, ]
# ml_data_test <- ml_data[-train_index, ]
# 
# # Ensure all predictor columns are numeric before converting to matrices
# predictor_columns <- colnames(ml_data_train)[!(colnames(ml_data_train) %in% c("disease_status", "time_to_event"))]
# non_numeric_predictors <- sapply(ml_data_train[predictor_columns], is.numeric) == FALSE
# 
# if (any(non_numeric_predictors)) {
#   stop("Error: All predictors must be numeric. Please convert these columns to numeric or exclude them.")
# }
# 
# # Define the predictor matrix and response vector for training and testing sets
# X_train <- as.matrix(ml_data_train %>% select(-disease_status, -time_to_event))
# y_train <- ml_data_train$disease_status
# 
# X_test <- as.matrix(ml_data_test %>% select(-disease_status, -time_to_event))
# y_test <- ml_data_test$disease_status
# 
# # Define a cross-validation control
# cv_control <- trainControl(method = "cv", number = 10, savePredictions = TRUE, classProbs = TRUE)
# 
# # Train the models: LASSO, Ridge, and Elastic Net
# lasso_model <- train(
#   x = X_train,
#   y = y_train,
#   method = "glmnet",
...
# )
# 
# # Function to calculate Harrell's C-statistic
# calculate_c_statistic <- function(model, X_test, y_test) {
#   predictions <- predict(model$finalModel, s = model$bestTune$lambda, newx = X_test, type = "response")
#   cox_model <- coxph(Surv(time_to_event, as.numeric(y_test)) ~ predictions, data = ml_data_test)
#   return(summary(cox_model)$concordance[1])
# }
# 
# # Calculate AUC and Harrell's C-statistic for each model
# auc_lasso <- auc(roc(y_test, as.numeric(predict(lasso_model$finalModel, s = lasso_model$bestTune$lambda, newx = X_test, type = "response"))))
# c_stat_lasso <- calculate_c_statistic(lasso_model, X_test, y_test)
# 
# auc_ridge <- auc(roc(y_test, as.numeric(predict(ridge_model$finalModel, s = ridge_model$bestTune$lambda, newx = X_test, type = "response"))))
# c_stat_ridge <- calculate_c_statistic(ridge_model, X_test, y_test)
# 
# auc_elastic_net <- auc(roc(y_test, as.numeric(predict(elastic_net_model$finalModel, s = elastic_net_model$bestTune$lambda, newx = X_test, type = "response"))))
# c_stat_elastic_net <- calculate_c_statistic(elastic_net_model, X_test, y_test)
# 
# # Print the AUC and Harrell's C-statistic values for each model
# cat("\nModel Performance Metrics:\n")
# cat("\nLASSO Model:\n")
# cat("AUC: ", auc_lasso, "\n")
# cat("Harrell's C-Statistic: ", c_stat_lasso, "\n")
# 
# cat("\nRidge Model:\n")
# cat("AUC: ", auc_ridge, "\n")
# cat("Harrell's C-Statistic: ", c_stat_ridge, "\n")
# 
# cat("\nElastic Net Model:\n")
# cat("AUC: ", auc_elastic_net, "\n")
# cat("Harrell's C-Statistic: ", c_stat_elastic_net, "\n")
# 
# # Determine the best model based on AUC
# auc_values <- c(LASSO = auc_lasso, Ridge = auc_ridge, ElasticNet = auc_elastic_net)
# best_auc_model_name <- names(which.max(auc_values))
# best_auc_value <- max(auc_values)
# 
# cat("\nBest Model based on AUC:\n")
# cat(best_auc_model_name, "with AUC:", best_auc_value, "\n")
# 
# # Save model performance metrics
# model_performance <- data.frame(
#   Model = c("LASSO", "Ridge", "ElasticNet"),
#   AUC = c(auc_lasso, auc_ridge, auc_elastic_net),
#   Harrell_C = c(c_stat_lasso, c_stat_ridge, c_stat_elastic_net)
# )
# 
...
# write.csv(model_performance, "model_performance_with_cstat_auc.csv", row.names = FALSE)
# 
# # Get the coefficients and variable importance for the best model (based on AUC)
# best_model <- switch(
#   best_auc_model_name,
#   "LASSO" = lasso_model,
#   "Ridge" = ridge_model,
...
# coefficients <- as.matrix(coef(best_model$finalModel, s = best_model$bestTune$lambda))
# 
...
# )
# 
# # Remove intercept from the importance_df
...
# 
# # Print the coefficient and importance table
# cat("\nVariable Importance (sorted by absolute coefficient value):\n")
# print(importance_df)
# 
# # Plot variable importance
...
#   coord_flip() +
#   labs(title = "Variable Importance", x = "Variables", y = "Importance (Absolute Coefficient Value)") +
#   theme_minimal()
# 
# # Print the time taken to run the model
# end_time <- Sys.time()
# execution_time <- end_time - start_time
# cat("\nTime taken to run the model:", execution_time, "\n")
...
# ml_data_normalized <- data
# 
# # Filter the data for race == 1
# ml_data_normalized <- ml_data_normalized %>% filter(race == 1)
# 
# # Convert disease_status to a factor with two levels: "Class0" and "Class1"
# ml_data_normalized <- ml_data_normalized %>%
#   mutate(disease_status = factor(disease_status, levels = c(0, 1), labels = c("Class0", "Class1")))
# 
# # Check the levels of the disease_status column before any further processing
# cat("Levels of disease_status before processing:\n")
# print(levels(ml_data_normalized$disease_status))
# 
# # Ensure there are at least 2 data points for each class before splitting
# class_counts <- table(ml_data_normalized$disease_status)
# cat("\nClass counts before splitting:\n")
# print(class_counts)
# 
# if (any(class_counts < 2)) {
#   stop("Error: One of the classes has fewer than 2 data points before splitting. Please check your dataset.")
# }
# 
# # Prepare the data: Remove person_id and any other non-numeric variables if needed
# ml_data <- ml_data_normalized %>%
...
# ml_data_test <- ml_data[-train_index, ]
# 
# # Verify that `disease_status` is a factor with two levels in both training and testing sets
# cat("Training set disease_status levels:\n")
# print(levels(ml_data_train$disease_status))
# cat("Testing set disease_status levels:\n")
# print(levels(ml_data_test$disease_status))
# 
# # Inspect the distribution of classes in the training set
# cat("\nClass counts in the training set:\n")
# print(table(ml_data_train$disease_status))
# 
# # Inspect the distribution of classes in the testing set
# cat("\nClass counts in the testing set:\n")
# print(table(ml_data_test$disease_status))
# 
# # Ensure all predictor columns are numeric before converting to matrices
# predictor_columns <- colnames(ml_data_train)[!(colnames(ml_data_train) %in% c("disease_status", "time_to_event"))]
# non_numeric_predictors <- sapply(ml_data_train[predictor_columns], is.numeric) == FALSE
...
#   print(predictor_columns[non_numeric_predictors])
#   stop("Error: All predictors must be numeric. Please convert these columns to numeric or exclude them.")
# }
# 
# # Define the predictor matrix and response vector for training and testing sets
...
# y_test <- ml_data_test$disease_status
# 
# # Check for NA values in the predictor matrices
# if (any(is.na(X_train))) {
#   cat("\nNA values detected in the training predictor matrix. Please handle them.\n")
#   print(which(is.na(X_train), arr.ind = TRUE))  # Print locations of NA values
#   stop("Please handle NA values in the training predictor matrix before proceeding.")
# }
# 
# if (any(is.na(X_test))) {
#   cat("\nNA values detected in the testing predictor matrix. Please handle them.\n")
#   print(which(is.na(X_test), arr.ind = TRUE))  # Print locations of NA values
#   stop("Please handle NA values in the testing predictor matrix before proceeding.")
# }
# 
# # Ensure `y_train` and `y_test` are factors with two levels after defining them
# cat("\nCheck factor levels after splitting and matrix creation:\n")
# print(levels(y_train))
# print(levels(y_test))
# 
# if (!is.factor(y_train) || length(levels(y_train)) != 2) {
#   stop("Error: The training response variable 'y_train' must be a factor with two levels.")
# }
# if (!is.factor(y_test) || length(levels(y_test)) != 2) {
#   stop("Error: The test response variable 'y_test' must be a factor with two levels.")
# }
# 
# # Define a cross-validation control
# cv_control <- trainControl(method = "cv", number = 10, savePredictions = TRUE, classProbs = TRUE)
# 
# # 1. LASSO Model (alpha = 1)
# lasso_model <- train(
#   x = X_train,
#   y = y_train,
#   method = "glmnet",
...
# )
# 
# # 2. Ridge Regression Model (alpha = 0)
# ridge_model <- train(
#   x = X_train,
#   y = y_train,
#   method = "glmnet",
...
# )
# 
# # 3. Elastic Net Model (alpha between 0 and 1)
# elastic_net_model <- train(
#   x = X_train,
#   y = y_train,
#   method = "glmnet",
...
# )
# 
# # Print the best models for LASSO, Ridge, and Elastic Net
# cat("Best LASSO Model:\n")
...
# 
# # Compare the models based on their cross-validation performance
# best_model <- list(
#   LASSO = lasso_model,
#   Ridge = ridge_model,
#   ElasticNet = elastic_net_model
# )
# 
# # Determine the best model based on Accuracy or another relevant metric
# best_overall_model <- best_model[[which.max(c(max(lasso_model$results$Accuracy, na.rm = TRUE), 
#                                               max(ridge_model$results$Accuracy, na.rm = TRUE), 
#                                               max(elastic_net_model$results$Accuracy, na.rm = TRUE)))]]
# cat("\nBest Overall Model:\n")
# print(best_overall_model)
# 
# # Use the best overall model for final predictions
# final_model <- best_overall_model$finalModel
# lambda_best <- best_overall_model$bestTune$lambda
# alpha_best <- if ("alpha" %in% names(best_overall_model$bestTune)) best_overall_model$bestTune$alpha else NA
# 
# # Predictions on the test set
# predictions <- predict(final_model, s = lambda_best, newx = X_test, type = "response")
# predicted_classes <- ifelse(predictions > 0.5, "Class1", "Class0")
# 
...
# print(model_performance)
# 
# # Save the performance metrics to a CSV file for publication
# write.csv(model_performance, "model_performance_lancet_race1.csv", row.names = FALSE)
# 
# # Print the time taken to run the model
# end_time <- Sys.time()
# execution_time <- end_time - start_time
...
# 
# # Filter the data for race == 1
# ml_data_normalized <- ml_data_normalized %>% filter(race == 0)
# 
# # Convert disease_status to a factor with two levels: "Class0" and "Class1"
# ml_data_normalized <- ml_data_normalized %>%
#   mutate(disease_status = factor(disease_status, levels = c(0, 1), labels = c("Class0", "Class1")))
...
Variable Importance and Model Equation

1.  **Variable Importance for the Best Model:** The best model is a linear model with regularization (Elastic Net, in your case). The coefficients of this model indicate the importance of each variable. The larger the absolute value of a coefficient, the more important the variable is in predicting the outcome.

2.  **Model Equation:** The final model equation is a linear combination of the variables, weighted by their coefficients. This equation can be used for future predictions on new data.

\`\`\`{# {r} \# \# Assuming you have already trained your models: lasso_model, ridge_model, elastic_net_model \# \# \# Function to calculate AUC for a given model \# calculate_auc \<- function(model, X_test, y_test) { \# predictions \<- predict(model$finalModel, s = model$bestTune$lambda, newx = X_test, type = "response") # roc_curve <- roc(y_test, as.numeric(predictions)) # auc_value <- auc(roc_curve) # return(auc_value) # } # # # Calculate AUC for LASSO # auc_lasso <- calculate_auc(lasso_model, X_test, y_test) # cat("\nAUC for LASSO Model:", auc_lasso, "\n") # # # Calculate AUC for Ridge Regression # auc_ridge <- calculate_auc(ridge_model, X_test, y_test) # cat("\nAUC for Ridge Regression Model:", auc_ridge, "\n") # # # Calculate AUC for Elastic Net # auc_elastic_net <- calculate_auc(elastic_net_model, X_test, y_test) # cat("\nAUC for Elastic Net Model:", auc_elastic_net, "\n") # # # Combine the AUCs into a data frame for easier comparison # auc_df <- data.frame( # Model = c("LASSO", "Ridge", "Elastic Net"), # AUC = c(auc_lasso, auc_ridge, auc_elastic_net) # ) # # # Print the AUCs # cat("\nAUCs for All Models:\n") # print(auc_df) # # # Save the AUCs to a CSV file for future reference # write.csv(auc_df, "model_aucs_lancet.csv", row.names = FALSE) # # # Proceed with the coefficient extraction and variable importance as before # # Extract the coefficients from the best model (assuming final_model is defined) # coefficients <- coef(final_model, s = lambda_best) # # # Convert the coefficients to a data frame for easier manipulation # coefficients_df <- as.data.frame(as.matrix(coefficients)) # colnames(coefficients_df) <- c("Coefficient") # coefficients_df$Variable \<- rownames(coefficients_df) \# \# \# Remove the intercept (if present) \# coefficients_df \<- coefficients_df\[coefficients_df$Variable != "(Intercept)", ] # # # Sort the coefficients in descending order of absolute importance # coefficients_df <- coefficients_df %>% # mutate(Importance = abs(Coefficient)) %>% # arrange(desc(Importance)) # # # Print the coefficients in descending order of importance # cat("\nVariable Importance in Descending Order:\n") # print(coefficients_df[, c("Variable", "Coefficient", "Importance")]) # # # Model equation in the form y = beta_0 + beta_1*x_1 + beta_2*x_2 + ... # cat("\nModel Equation:\n") # model_equation <- paste0("y = ", round(as.numeric(coef(final_model, s = lambda_best)[1]), 4), # " + ", paste0(round(coefficients_df$Coefficient, 4), " \* ", coefficients_df\$Variable, collapse = " + ")) \# cat(model_equation, "\n") \# \# \# Save the coefficients to a CSV file for future reference \# write.csv(coefficients_df, "variable_importance_lancet.csv", row.names = FALSE)

```         

**2. Random Forest:** Tune the number of trees, max depth, and minimum samples per leaf.

```f3175f2a-e8a0-4436-be12-b33925b6d220
31b8e172-b470-440e-83d8-e6b185028602::31b8e172-b470-440e-83d8-e6b185028602
```

**3. XGBoost:** Tune the learning rate, max depth, and number of estimators.

\`\`\`{# {r} \# \# Load necessary libraries \# library(xgboost) \# library(caret) \# library(dplyr) \# library(pROC) \# \# ml_data_xgb \<- data_reserve \# \# \# Start timing the execution \# start_time \<- Sys.time() \# \# \# \# Convert the data to a format suitable for XGBoost \# X \<- as.matrix(ml_data_xgb %\>% select(-disease_status, -time_to_event)) \# Predictor variables \# y \<- as.numeric(ml_data_xgb$disease_status) - 1 # Response variable (binary, 0/1) # # # Split data into training and testing sets # set.seed(123) # For reproducibility # train_index <- createDataPartition(y, p = 0.7, list = FALSE) # X_train <- X[train_index, ] # y_train <- y[train_index] # X_test <- X[-train_index, ] # y_test <- y[-train_index] # # # Convert data to XGBoost's DMatrix format # dtrain <- xgb.DMatrix(data = X_train, label = y_train) # dtest <- xgb.DMatrix(data = X_test, label = y_test) # # # Define parameters for the XGBoost model # params <- list( # objective = "binary:logistic", # Binary classification # eval_metric = "auc", # Evaluation metric (Area Under the Curve) # max_depth = 6, # Maximum depth of a tree # eta = 0.01, # Learning rate # subsample = 0.8, # Subsample ratio of the training instances # colsample_bytree = 0.8, # Subsample ratio of columns when constructing each tree # gamma = 0, # Minimum loss reduction required to make a further partition # lambda = 1, # L2 regularization term on weights # alpha = 0 # L1 regularization term on weights # ) # # # Train the XGBoost model # xgb_model <- xgb.train( # params = params, # data = dtrain, # nrounds = 1000, # Number of boosting rounds # watchlist = list(train = dtrain, test = dtest), # early_stopping_rounds = 10, # Stop if no improvement after 10 rounds # verbose = 1 # Print the progress # ) # # # Print the model summary # cat("\nXGBoost Model Summary:\n") # print(xgb_model) # # # Predict on the test set # xgb_predictions <- predict(xgb_model, dtest) # predicted_classes <- ifelse(xgb_predictions > 0.5, 1, 0) # # # Evaluate the model's performance # confusion_matrix_xgb <- confusionMatrix(as.factor(predicted_classes), as.factor(y_test)) # cat("\nConfusion Matrix:\n") # print(confusion_matrix_xgb) # # # Calculate AUC for the XGBoost model # roc_curve_xgb <- roc(y_test, xgb_predictions) # auc_value_xgb <- auc(roc_curve_xgb) # cat("\nAUC (Area Under the Curve) for XGBoost:\n") # print(auc_value_xgb) # # # Save model performance metrics # model_performance_xgb <- data.frame( # Accuracy = confusion_matrix_xgb$overall\["Accuracy"\], \# Sensitivity = confusion_matrix_xgb$byClass["Sensitivity"], # Specificity = confusion_matrix_xgb$byClass\["Specificity"\], \# Precision = confusion_matrix_xgb$byClass["Pos Pred Value"], # F1_Score = confusion_matrix_xgb$byClass\["F1"\], \# AUC = auc_value_xgb \# ) \# \# cat("\nModel Performance Metrics for XGBoost:\n") \# print(model_performance_xgb) \# \# \# Save the performance metrics to a CSV file for publication \# write.csv(model_performance_xgb, "model_performance_xgb_lancet.csv", row.names = FALSE) \# \# \# Plot feature importance \# importance_matrix \<- xgb.importance(feature_names = colnames(X_train), model = xgb_model) \# cat("\nFeature Importance:\n") \# print(importance_matrix) \# \# \# Plot the feature importance \# xgb.plot.importance(importance_matrix, main = "Feature Importance in XGBoost") \# \# \# Print the time taken to run the model \# end_time \<- Sys.time() \# execution_time_xgb \<- end_time - start_time \# cat("\nTime taken to run the XGBoost model:", execution_time_xgb, "\n")

```         

**4. SVM:** Tune the penalty parameter CCC and the kernel type.
...
# start_time <- Sys.time()
# 
# ml_data <- data_reserve
# 
# # Check the levels of disease_status to ensure it's correctly formatted
# cat("Levels of disease_status:\n")
# print(levels(ml_data$disease_status))
# 
# # Split data into training and testing sets
# set.seed(123)  # For reproducibility
# train_index <- createDataPartition(ml_data$disease_status, p = 0.8, list = FALSE)
# train_data <- ml_data[train_index, ]
# test_data <- ml_data[-train_index, ]
# 
# # Define the predictor matrix and response vector
# X_train <- train_data %>% select(-disease_status)  # Predictor variables for training
# y_train <- train_data$disease_status  # Response variable for training
# 
# X_test <- test_data %>% select(-disease_status)  # Predictor variables for testing
# y_test <- test_data$disease_status  # Response variable for testing
# 
...
#   gamma = 1/ncol(X_train),  # Kernel coefficient
...
# probabilities <- attr(predictions, "probabilities")[, "Class1"]
# roc_curve <- roc(as.numeric(y_test), probabilities)
# auc_value <- auc(roc_curve)
# cat("\nAUC (Area Under the Curve):\n")
# print(auc_value)
# 
...
# 
# # Save the performance metrics to a CSV file for publication
# write.csv(model_performance, "svm_model_performance_lancet.csv", row.names = FALSE)
# 
# # Print the time taken to run the model
# end_time <- Sys.time()
# execution_time <- end_time - start_time
# cat("\nTime taken to run the SVM model:", execution_time, "\n")
```

**5. Neural Nets:** Tune the number of layers, number of neurons per layer, learning rate, and activation functions.

\`\`\`{# {r} \# \# Load necessary libraries \# library(neuralnet) \# For neural networks \# library(caret) \# For cross-validation and model evaluation \# library(dplyr) \# \# ml_data_normal \<- data_reserve \# \# \# Start timing the execution \# start_time \<- Sys.time() \# \# \# Sanitize column names to ensure they are valid for formulas \# colnames(ml_data_normal) \<- make.names(colnames(ml_data_normal)) \# \# \# Prepare the data: Convert 'disease_status' to a numeric factor (0, 1) \# ml_data \<- ml_data_normal %\>% \# mutate(disease_status = as.numeric(disease_status) - 1) %\>% \# select(-time_to_event) \# Remove time_to_event column \# \# \# Split data into training and testing sets \# set.seed(123) \# For reproducibility \# train_index \<- createDataPartition(ml_data$disease_status, p = 0.7, list = FALSE) # train_data <- ml_data[train_index, ] # test_data <- ml_data[-train_index, ] # # # Split predictors into smaller groups to avoid long formula issues # predictor_chunks <- split(names(train_data)[!names(train_data) %in% "disease_status"], ceiling(seq_along(names(train_data)[!names(train_data) %in% "disease_status"]) / 20)) # # # Define and train neural network models for each chunk # nn_models <- lapply(predictor_chunks, function(chunk) { # nn_formula <- as.formula(paste("disease_status ~", paste(chunk, collapse = " + "))) # # neuralnet( # formula = nn_formula, # data = train_data, # hidden = c(10, 5), # Two hidden layers with 10 and 5 neurons respectively # linear.output = FALSE, # Sigmoid activation function (binary output) # stepmax = 1e6, # Max number of steps to train the network # lifesign = "minimal" # Print minimal information during training # ) # }) # # # Combine the predictions from each chunk # nn_predictions <- sapply(nn_models, function(model) { # compute(model, test_data %>% select(all_of(attr(model$covariate, "names"))))$net.result # }) # nn_predictions_combined <- rowMeans(nn_predictions) # # # Predict the final classes # predicted_classes <- ifelse(nn_predictions_combined > 0.5, 1, 0) # # # Evaluate the model's performance # confusion_matrix <- confusionMatrix(as.factor(predicted_classes), as.factor(test_data$disease_status)) \# cat("\nConfusion Matrix:\n") \# print(confusion_matrix) \# \# \# Calculate and print the AUC (Area Under the Curve) \# library(pROC) \# roc_curve \<- roc(test_data$disease_status, nn_predictions_combined) # auc_value <- auc(roc_curve) # cat("\nAUC (Area Under the Curve):\n") # print(auc_value) # # # Save model performance metrics # model_performance <- data.frame( # Accuracy = confusion_matrix$overall\["Accuracy"\], \# Sensitivity = confusion_matrix$byClass["Sensitivity"], # Specificity = confusion_matrix$byClass\["Specificity"\], \# Precision = confusion_matrix$byClass["Pos Pred Value"], # F1_Score = confusion_matrix$byClass\["F1"\], \# AUC = auc_value \# ) \# \# cat("\nModel Performance Metrics:\n") \# print(model_performance) \# \# \# Save the performance metrics to a CSV file for publication \# write.csv(model_performance, "nn_model_performance_lancet.csv", row.names = FALSE) \# \# \# Print the time taken to run the model \# end_time \<- Sys.time() \# execution_time \<- end_time - start_time \# cat("\nTime taken to run the Neural Network model:", execution_time, "\n")
