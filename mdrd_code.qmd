...
measurements <- list(
  ...
  "Creatinine [Mass/volume] in Serum or Plasma", #5
...
```{r}
# Step 1: Filter the measurements for "Creatinine [Mass/volume] in Serum or Plasma"
creatinine_measurements <- filtered_measurements %>%
  filter(standard_concept_name == "Creatinine [Mass/volume] in Serum or Plasma")

# Step 2: For each person_id, select the first creatinine measurement based on the earliest measurement_datetime
first_creatinine <- creatinine_measurements %>%
  group_by(person_id) %>%
  arrange(measurement_datetime) %>%   # Ensure the earliest measurement comes first
  slice(1) %>%                        # Select the first row per person_id
  ungroup() %>%
  select(person_id, serum_creatinine = value_as_number)  # Rename the column to "creatinine"

# Step 3: Join the first creatinine values back to the person_filtered dataset using person_id
person_filtered <- person_filtered %>%
  left_join(first_creatinine, by = "person_id")  # Add creatinine data based on person_id

# Step 4: Print the resulting dataset to verify the new column
print(person_filtered)
...
#### Model evaluation: Bootstrap 10,000 random sampling
...
Show confidence interval of AUC, sample without replavcement, is there a way to calculate the confidence interval based on 1 individual sample., 95/ CI of AUC, with the actual AUC in the enytire data.
